/**
 * Created by oyqy on 2016/9/9.
 */

app.controller('apiDetailCtrl',['$scope','globalConfig','$rootScope','dataService','$timeout','$state',
    function($scope,globalConfig,$rootScope,dataService,$timeout,$state){
        ...
        $scope.$watch('dataVal', function (newValue) {
            if($scope.dataVal==undefined) {
                dataService.getData('getLabelListTree',{
                    rootId: 24
        ...
        //点击事件
        $('.apiDetailCtrl .clear').on('click',function(e){
            var text = e.target.text;
            if(text=='3.0'){
                $(".apiDetailCtrl .api-label li").removeClass("on");
                $('#api30').addClass('on');
            }else  if(text=='2.0'){
                $(".apiDetailCtrl .api-label li").removeClass("on");
                $('#api20').addClass('on');
